...
if (formChatData) {
  formChatData.addEventListener("submit", (e) => {
    ...
    if (content || images.length > 0) {
      const roomId = document.querySelector('.chat-body')?.getAttribute('data-room-id') || '';
...
if (elementListTying) {
  socket.on("SERVER_RETURN_TYPING", (data) => {
    if (data.type == "show") {
      const exitsTyping = elementListTying.querySelector(
        `[user-id="${data.userID}"]`
      );
      if (!exitsTyping) {
      ...
      const BoxTyingRemove = elementListTying.querySelector(
        `[user-id="${data.userID}"]`
      );
      if (BoxTyingRemove) {
