...
const Accounts = require("../../models/Accounts.model"); // Add this line
const sortHelper = require("../../helper/sort"); // Add this line
const index = async (req, res) => {
  const { keyword, status, page, sort } = req.query;
...
const changeStatus = async (req, res) => {
  ...
  await Category.updateOne(
    { _id: id },
    {
      status: status,
      $push: {
        updatedBy: { account_id: res.locals.user.id, updatedAt: Date.now() },
      },
    }
  );
  res.redirect(`${system.prefixAdmin}/categories`);
